...
func (s testStorageImpl) StoreUserRequest(ctx context.Context, apptoken, username string, req *opaqueserver.UserRequest) error {
...
func (s testStorageImpl) LoadUserRequest(ctx context.Context, apptoken, username string) (*opaqueserver.UserRequest, error) {
...
func (s testStorageImpl) StoreUserEnvelope(ctx context.Context, apptoken, username string, req *opaqueserver.UserEnvelope) error {
	b, err := json.Marshal(req)
	if err != nil {
		return errors.Wrap(err, "")
	}
...
func (s testStorageImpl) LoadUserEnvelope(ctx context.Context, apptoken, username string) (*opaqueserver.UserEnvelope, error) {
...
func doPasswordRegistration(ctx context.Context, s *opaqueserver.Server, username, password string) error {
	// 1. Client starts the OPRF process
	_, finData, evalReq, err := opaqueclient.MakeOprfRequest(password)
	if err != nil {
		return err
	}
	...
	envU, envUNonce, pubU, salt, err := opaqueclient.MakeEnvU(finData, sEval, s.PubS)
	if err != nil {
		return err
	}
	fmt.Printf("envU = %+v\n", envU)
...
func TestProtocol(t *testing.T) {
	t.Run("Happy path: register -> login -> access private resource", func(t *testing.T) {
		...
		s, err := opaqueserver.NewServer(testStorageImpl{miniredis.RunT(t)}, nil)
